...
// export const getDevHistory = (bodyParser.urlencoded(), async(req: Request, res: Response, next: NextFunction) => 
// {
//   const api_key = process.env.API_KEY
//   const graphqlEndpoint = process.env.API_ENDPOINT || ''
...
const getPreviousCoins = async (address) => {
  const connection = new Connection("https://api.mainnet-beta.solana.com");
  const publicKey = new PublicKey(address);

  try {
    // Fetch the signatures for transactions involving the given address
    ...
    for (const signature of signatures) {
      ...
      if (transaction && transaction.meta?.postTokenBalances) {
        console.log("Transaction Signature:", signature.signature);
        console.log("Tokens Created or Affected:", transaction.meta.postTokenBalances);
      }
    }
  } catch (error) {
...
{
  let data = getPreviousCoins(req.body.devAddress)
  return res.status(200).send({
    data: data
  })
})
